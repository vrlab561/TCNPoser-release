...
def train_loop(configs, device, model, loss_func, optimizer, \
    ...
    for epoch in range(configs.train_config.epochs):
        ...
        one_epoch_root_loss, one_epoch_lpose_loss, one_epoch_gpose_loss, one_epoch_joint_loss, \
            one_epoch_acc_loss, one_epoch_shape_loss, one_epoch_root_euler_loss, one_epoch_rootY_euler_loss, one_epoch_total_loss = [], [], [], [], [], [], [],[],[]
        for batch_idx, (input_feat, gt_local_pose, gt_global_pose, gt_positions, gt_betas) in enumerate(train_loader):
            ...
            pred_local_pose, pred_betas, rotation_global_r6d, pred_joint_position,pred_local_pose_aa = model(input_feat)
            ...
            root_orientation_loss, local_pose_loss, global_pose_loss, joint_position_loss, accel_loss, shape_loss,loss_root_euler,loss_rootY_euler, total_loss = \
            ...
            one_epoch_root_euler_loss.append(loss_root_euler.item())
            one_epoch_rootY_euler_loss.append(loss_rootY_euler.item())
            ...
            if batch_idx % configs.train_config.log_interval == 0:
                batch_info = {
                    ...
                    'root_euler_loss': round(float(loss_root_euler.item()), 3),
                    'rootY_euler_loss': round(float(loss_rootY_euler.item()), 3),
        ...
        one_epoch_root_euler_loss = torch.tensor(one_epoch_root_euler_loss).mean().item()
        one_epoch_rootY_euler_loss = torch.tensor(one_epoch_rootY_euler_loss).mean().item()
        ...
        train_summary_writer.add_scalar(
            'train_epoch_loss/joint_3d_loss', one_epoch_root_euler_loss, epoch)
        train_summary_writer.add_scalar(
            'train_epoch_loss/smooth_loss', one_epoch_rootY_euler_loss, epoch)
        train_summary_writer.add_scalar(
        ...
        epoch_info = {
            ...
            'root_euler_loss': round(float(one_epoch_root_euler_loss), 3),
            'rootY_euler_loss': round(float(one_epoch_rootY_euler_loss), 3),
        ...
        if epoch % configs.train_config.val_interval == 0:
            ...
            with torch.no_grad():
                for _, (input_feat, gt_local_pose, gt_global_pose, gt_positions, gt_betas) in tqdm(
                    ...
                    pos_error_ = torch.mean(torch.sqrt(
                        torch.sum(torch.square(gt_positions_head_centered - pred_joint_position_head_centered), axis=-1)
                    ))
            ...
            train_summary_writer.add_scalar(
                'val_epoch_loss/joint_3d_loss', one_epoch_root_euler_loss, epoch)
            train_summary_writer.add_scalar(
                'val_epoch_loss/smooth_loss', one_epoch_rootY_euler_loss, epoch)
