...
metrics_coeffs = {
    ...
    "R-RE": RADIANS_TO_DEGREES,      #R-RE
    "R-RE-VERT": RADIANS_TO_DEGREES,    #R-RE-VERT
    "pred_jitter": 1.0,
    "gt_jitter": 1.0,
}
...
def main():
    ...
    model.load_state_dict(torch.load(configs.resume_model, map_location=device))
    ...
    for input_type in configs.compatible_inputs:
        ...
        with torch.no_grad():
            for _, (input_feat, gt_local_pose, gt_global_pose, gt_positions, head_global_trans) in tqdm(
                ...
                input_feat, gt_local_pose, gt_global_pose, gt_positions, head_global_trans = input_feat.to(
                    device).float(), \
                ...
                pred_local_pose, _, rotation_global_r6d, pred_joint_position, pred_local_pose_aa = model(input_feat)
                rotation_local_matrot = utils_transform.sixd2matrot(pred_local_pose.reshape(-1, 6)).reshape(
                    batch_size * time_seq, 22, 3, 3)
                ...
                gt_local_pose_matrot = utils_transform.sixd2matrot(gt_local_pose.reshape(-1, 6)).reshape(
                    batch_size * time_seq, 22, 3, 3)

                len1 = rotation_local_matrot.size(0)
                pred_local_pose_euler = []
                gt_local_pose_euler = []
                for i in range(len1):
                    pre_list1 = euler.mat2euler(rotation_local_matrot[i][0].cpu())
                    gt_list1 = euler.mat2euler(gt_local_pose_matrot[i][0].cpu())

                    pred_local_pose_euler.append(torch.tensor(pre_list1))
                    gt_local_pose_euler.append(torch.tensor(gt_list1))

                predicted_root_euler = torch.stack(pred_local_pose_euler)
                gt_root_euler = torch.stack(gt_local_pose_euler)
