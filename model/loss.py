...
class PoseJointLoss(nn.Module):
    def __init__(self, loss_type, root_orientation_weight=1.0,
                ...
                root_euler_weight=1.0,
                rootY_euler_weight=1.0
    ) -> None:
        super().__init__()
        ...
        self.root_euler_weight = root_euler_weight
        self.rootY_euler_weight = rootY_euler_weight
        if loss_type == 'l1':
    ...
    def forward(self, pred_root_orientation,
        ...
        gt_local_pose_matrot = utils_transform.sixd2matrot(gt_root_orientation.reshape(-1, 6)).reshape(
            gt_root_orientation.shape[0] * gt_root_orientation.shape[1], 1, 3, 3)
        pred_root_orientation_matrot = utils_transform.sixd2matrot(pred_root_orientation.reshape(-1, 6)).reshape(
            pred_root_orientation.shape[0] * pred_root_orientation.shape[1], 1, 3, 3)
        len1 = gt_local_pose_matrot.size(0)
        pred_local_pose_euler = []
        gt_local_pose_euler = []
        for i in range(len1):
            pre_list1 = []
            gt_list1 = []
            pre_list1 = euler.mat2euler(pred_root_orientation_matrot[i][0].cpu().detach().numpy())
            gt_list1 = euler.mat2euler(gt_local_pose_matrot[i][0].cpu().detach().numpy())

            pred_local_pose_euler.append(torch.tensor(pre_list1))
            gt_local_pose_euler.append(torch.tensor(gt_list1))

        predicted_root_euler = torch.tensor(np.array(pred_local_pose_euler))
        gt_root_euler = torch.tensor(np.array(gt_local_pose_euler))

        loss_root_euler = torch.mean(torch.abs(predicted_root_euler[:,0] - gt_root_euler[:,0])+torch.abs(predicted_root_euler[:,1] - gt_root_euler[:,1])+torch.abs(predicted_root_euler[:,2] - gt_root_euler[:,2]))
        loss_rootY_euler = torch.mean(torch.abs(predicted_root_euler[:, 2] - gt_root_euler[:, 2]))
        ...
        self.root_euler_weight * loss_root_euler +
        self.rootY_euler_weight * loss_rootY_euler
        )

        return root_orientation_loss, local_pose_loss, global_pose_loss, joint_position_loss, accel_loss, shape_loss,loss_root_euler,loss_rootY_euler, total_loss