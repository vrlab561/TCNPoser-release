...
class TemporalSpatialBackbone(torch.nn.Module):
    def __init__(self, input_dim, number_layer=3, hidden_size=256, dropout=0.05, nhead=8, block_num=2):
        ...
        self.cbam_layers = nn.ModuleList(
            [CBAM(8) for _ in range(self.num_block)]
        )
    ...
    def forward(self, x_in, rnn_state=None):
        ...
        for idx in range(self.num_block):
            ...
            cbam_input = collect_feats_temporal.permute(0, 2, 1, 3) 

            cbam_output = self.cbam_layers[idx](cbam_input)

            collect_feats_temporal = cbam_output.reshape(batch_size, 8, time_seq, -1)
            collect_feats_temporal = collect_feats_temporal.permute(0, 2, 1, 3)
...
class CBAM(nn.Module): 
    def __init__(self, channels, reduction=1, spatial_kernel=7):
